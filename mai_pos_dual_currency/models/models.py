...
class ReportSaleDetails(models.AbstractModel):
    ...
    def get_sale_details(self, date_start=False, date_stop=False, config_ids=False, session_ids=False):
    	...
    	usd_crncy = self.env['res.currency'].sudo().search([
    		('symbol','=','$')],limit=1)
    	if usd_crncy:
    		data.update({'usd_rate' : usd_crncy.rate})
    	return data
...
class ResConfigSettings(models.TransientModel):
	...
	show_currency_rate = fields.Float(related='pos_config_id.show_currency_rate',readonly=True)
